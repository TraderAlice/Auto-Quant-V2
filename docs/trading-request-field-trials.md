...
| `book-reallocation` | `us-megacap-book-reallocation-v060-gap` | active — semantic gap reproduced before Run | AutoQuant `0.6.0` intake accepts the exact question, baseline snapshot, five request-authorized assets, and the same 643-session Yahoo package. The Project validates and orientation retains fixed descriptive/no-trading authority, but the derived snapshot contains only the reported four-asset baseline. No public contract binds the two complete TSLA-funded books, and the existing reduction table moves one percentage point only to cash. The Agent stopped before executing a baseline-only Run that would appear successful while failing to compare the caller's proposals. |
