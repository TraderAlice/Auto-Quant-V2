"""Fetch auditable recent raw A-share daily OHLCV from Sina Finance."""
...
PROVIDER_SYMBOL = re.compile(r"^(sh|sz|bj)[0-9]{6}$")
PREFIX = {"XSHG": "sh", "XSHE": "sz", "XBSE": "bj"}
DATA_LENGTH = 1023
...
def load_assets(path: Path) -> list[dict[str, str]]:
    ...
    for index, item in enumerate(raw):
        ...
        expected_prefix = PREFIX.get(normalized["venue"])
        if expected_prefix is None:
            raise ValueError(f"assets[{index}].venue is unsupported")
        if not normalized["providerSymbol"].startswith(expected_prefix):
            raise ValueError(f"assets[{index}] provider prefix/venue mismatch")
...
def request_uri(symbol: str) -> str:
...
def fetch_bytes(uri: str) -> tuple[bytes, dict[str, str]]:
    ...
    for attempt in range(5):
        try:
            ...
            if attempt == 4:
                raise RuntimeError(
                    "Sina route failed after 5 attempts: " + " | ".join(errors)
                ) from exc
...
def parse_payload(
    ...
    if not isinstance(status, dict) or int(status.get("code", -1)) != 0:
        raise ValueError(f"{provider_symbol}: provider status is not success")
    records = result.get("data")
    if not isinstance(records, list) or not records:
        raise ValueError(f"{provider_symbol}: response has no K-line rows")
    ...
    for index, row in enumerate(records):
        ...
        try:
            ...
            raise ValueError(
                f"{provider_symbol}: unusable row {index}"
            ) from exc
...
def main() -> None:
    ...
    audits: dict[str, Any] = {}
    for asset in assets:
        uri = request_uri(asset["providerSymbol"])
    ...
    package: dict[str, Any] = {
        ...
        "priceAdjustment": "raw",
        "provider": {
            "name": "sina-finance-observed-kline",
    ...
    audit = {
        ...
        "request": {
            ...
            "panel": args.panel,
            "adjustment": "raw",
            "requestedProviderRows": DATA_LENGTH,
        },
        "transformation": (
            "provider OHLC parsed as CNY per share; provider volume "
            "preserved as shares"
        ),
        "assets": audits,
        ...
        "limitations": [
            "Sina is an observable provider, not exchange authority.",
            "Only the most recent 1,023 provider sessions are requested.",
            "Venue, volume, access, and terms remain external claims.",
        ],
    }
